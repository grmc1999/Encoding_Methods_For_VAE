...
class Unpaired_Flexible_Encoding_Decoding_VAE_Decoupler(Base_Generative_AutoEncoder):
  ...
  def model(self,x_12,x_1,x_2): #TODO: INPUT SHOULD BE X_1 and X_2 decomposed images
    ...
    with pyro.plate("data",x_1.shape[0]):
      ...
      pyro.sample("obs_1",dist.Bernoulli(x_r_1.view(x_1.shape[0],x_1.shape[1]*x_1.shape[0])).to_event(3),obs=((x_1.x_1.shape[0],x_1.shape[1]*x_1.shape[0]>0.5)).float())
      pyro.sample("obs_2",dist.Bernoulli(x_r_2.view(x_1.shape[0],x_1.shape[1]*x_1.shape[0])).to_event(3),obs=((x_2.x_1.shape[0],x_1.shape[1]*x_1.shape[0]>0.5)).float())
      #pyro.sample("obs",dist.Bernoulli(x_r).to_event(1),obs=((x)).float())
